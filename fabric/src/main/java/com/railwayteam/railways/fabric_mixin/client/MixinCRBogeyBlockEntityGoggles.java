 ...
 * body, not just the interface - has to live here rather than on CRBogeyBlockEntity itself.
 */
@Mixin(CRBogeyBlockEntity.class)
@Implements(@Interface(iface = IHaveGoggleInformation.class, prefix = "goggle$"))
public abstract class MixinCRBogeyBlockEntityGoggles {
    public boolean goggle$addToGoggleTooltip(List<Component> tooltip, boolean isPlayerSneaking) {
        CRBogeyBlockEntity be = (CRBogeyBlockEntity) (Object) this;
        Lang.builder(Railways.MOD_ID)
                .add(Component.empty().append(be.getStyle().displayName).withStyle(ChatFormatting.GOLD))
                .forGoggles(tooltip);
        return true;
    }
}
