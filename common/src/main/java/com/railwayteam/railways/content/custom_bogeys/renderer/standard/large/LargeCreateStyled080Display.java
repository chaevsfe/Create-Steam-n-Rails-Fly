...
public class LargeCreateStyled080Display implements BogeyDisplay {
    ...
    public LargeCreateStyled080Display(ElementProvider<?> prov) {
        frame = prov.create(LARGE_CREATE_STYLED_0_8_0_FRAME);
        piston = prov.create(LARGE_CREATE_STYLED_0_8_0_PISTON);
        semiBlindWheels = prov.create(LC_STYLE_SEMI_BLIND_WHEELS, 2);
        wheels = prov.create(AllPartialModels.LARGE_BOGEY_WHEELS, 2);
        pins = prov.create(AllPartialModels.BOGEY_PIN, 4);
    ...
    public void update(CompoundTag bogeyData, float wheelAngle) {
        ...
        for (int side = 0; side < 4; side++) {
            pins[side]
                .translate(0, 1, -2.62f + side * 1.7467)
                .rotateXDegrees(wheelAngle)
                .translate(0, 1 / 4f, 0)
                .rotateXDegrees(-wheelAngle);
        }
