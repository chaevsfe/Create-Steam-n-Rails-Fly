...
public class SmokeStackMovementBehaviour extends MovementBehaviour {
    private static class TemporaryData {
        ...
        TemporaryData(MovementContext context) {
        ...
        void startMoving(MovementContext context) {
            movementStartTick = context.world.getGameTime();
        }

        long getMovementTicks(MovementContext context) {
        ...
        void addAndTrackParticle(ChimneyPushParticleData<?> particleType, boolean force, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            if (pushParticles == null) {
                pushParticles = new ArrayList<>();
            }
            Particle added = Minecraft.getInstance().particleEngine.createParticle(particleType, x, y, z, xSpeed, ySpeed, zSpeed);
    ...
    public void tick(MovementContext context) {
        ...
        if (context.blockEntityData != null) {
            int height = context.blockEntityData.getInt("height").orElse(0);
        ...
        if (context.blockEntityData != null) {
            if (context.blockEntityData.contains("color")) {
                color = DyeColor.byId(context.blockEntityData.getInt("color").orElse(0));
            }
            isSoul = context.blockEntityData.getBoolean("isSoul").orElse(false);
        }
        ...
        boolean emitSteam = CRConfigs.client().spawnSteam.get() || color == DyeColor.WHITE || isSoul;
        if (emitSteam) {
            int steamInterval = (int) Mth.clamp(6.25 / chanceModifier, 13, 50);
            if (context.world.getGameTime() % steamInterval <= 3) {
                if (data.getPushParticles().isEmpty()) {
                    ChimneyPushParticleData<?> particleType = ChimneyPushParticleData.create(false, false, DyeColor.WHITE);
                    Vec3 pos = context.position.add(data.offset).subtract(0.5, 0, 0.5).add(emissionParams.particleSpawnOffset());
                    data.addAndTrackParticle(particleType, true, pos.x, pos.y, pos.z, context.motion.x, context.motion.y, context.motion.z);
                }
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    emissionParams.makeParticles(
                        ...
                        random.nextBoolean(),
                        speedMultiplierChaser.getValue(), false,
                        isSoul ? null : DyeColor.WHITE, small, isSoul
                    );
                }
            }
        }
