...
public class LargeCreateStyled060Display implements BogeyDisplay {
    ...
    public LargeCreateStyled060Display(ElementProvider<?> prov) {
        frame = prov.create(LARGE_CREATE_STYLED_0_6_0_FRAME);
        piston = prov.create(LARGE_CREATE_STYLED_0_6_0_PISTON);
        semiBlindWheels = prov.create(LC_STYLE_SEMI_BLIND_WHEELS);
        wheels = prov.create(AllPartialModels.LARGE_BOGEY_WHEELS, 2);
        pins = prov.create(AllPartialModels.BOGEY_PIN, 3);
    ...
    public void update(CompoundTag bogeyData, float wheelAngle) {
        for (int side : Iterate.positiveAndNegative) {
            secondaryShafts[(side + 1) / 2]
                .translate(-.5f, .25f, -.5f + side * 2.681)
                .center()
