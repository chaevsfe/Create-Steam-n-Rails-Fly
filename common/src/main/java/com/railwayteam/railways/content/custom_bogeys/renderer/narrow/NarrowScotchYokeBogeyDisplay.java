...
public class NarrowScotchYokeBogeyDisplay implements BogeyDisplay {
    ...
    public NarrowScotchYokeBogeyDisplay(ElementProvider<?> prov) {
        frame = prov.create(NARROW_SCOTCH_FRAME);
        wheels = prov.create(NARROW_SCOTCH_WHEELS);
        pins = prov.create(NARROW_SCOTCH_WHEEL_PINS);
        pistons = prov.create(NARROW_SCOTCH_PISTONS);
    ...
    public void update(CompoundTag bogeyData, float wheelAngle) {
        ...
        pistons.translate(0, 5 / 16f, 1 / 4f * Math.sin(AngleHelper.rad(wheelAngle)));
