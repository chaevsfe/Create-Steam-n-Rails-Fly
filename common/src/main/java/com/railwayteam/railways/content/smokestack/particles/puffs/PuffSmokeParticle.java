...
public class PuffSmokeParticle extends CustomAnimatedTextureSheetParticle {
    ...
    protected PuffSmokeParticle(ClientLevel level, double x, double y, double z, SpriteSet spriteSet, RandomSource random, boolean stationarySource, double ySpeed) {
        super(level, x, y, z, 0.0, ySpeed, 0.0, spriteSet.first());
        if (Mth.equal(ySpeed, DOUBLE_SPEED_SENTINEL)) {
            this.yd *= 1.5;
        }
        this.quadSize = 1.0f;
        this.friction = 0.99f;
    ...
    protected double getAnimationProgress() {
        ...
        double fadeIn = 0.1;
        return this.stationarySource
            ? lifeFactor * lifeFactor
            : Math.max(0, ((lifeFactor - fadeIn) * (lifeFactor - fadeIn)) - (fadeIn * fadeIn));
    }
    ...
    public static class Factory<T extends PuffSmokeParticleData<T>> implements ParticleProvider<T> {
        ...
        public Particle createParticle(@NotNull T type, @NotNull ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, RandomSource random) {
            PuffSmokeParticle particle = new PuffSmokeParticle(level, x, y, z, spriteSet, random, type.stationary, ySpeed);
